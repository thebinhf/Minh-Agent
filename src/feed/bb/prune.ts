...
export function startPruner(config: TrackerConfig, store: TrackerDb) {
  const run = () => {
    ...
    console.log(
      `[minh:bb] prune ticker=${result.tickerDeleted} book=${result.bookDeleted} kline=${result.klineDeleted} oi=${result.oiDeleted} funding=${result.fundingDeleted} liq=${result.liqDeleted} flow=${result.flowDeleted}` +
