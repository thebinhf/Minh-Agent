...
Quant is a veto, not a signal: `tickers[].fundingRate`, `/map.funding.crowded`, `tickers[].openInterest`, `/map.oi.reading` (`long_add`/`short_add`/`cover`/`flush`), `/map.oi.deltaPct`, `/map.liq` (`below` = remaining long-liq under last, `cascade.active`), `tickers[].price24hPcnt`. Crowded long (`funding.crowded=long`) does not short; it blocks a weak demand arm. `oi.reading=short_add` into a demand zone → stand aside. `liq.cascade.active && side=long` + demand → wait for reclaim, do not catch the knife. `cascade.fuel` still large means the loop is not done. Scan 30m is HTTP `/confirm` + `/zones`. For live liq/ticker without polling: `ws://127.0.0.1:43180/ws` subscribe `liq.*` + `ticker.BTCUSDT`. Still do not arm from a print. `GET /liq-model` is a **labeled estimate** (not prints). Do not treat bright bins as targets; do not arm from it.
