...
| --- | --- | --- | --- |
| **MAP** | 1H/4H candle close | `GET /map?symbols=` or `bun run map BTCUSDT ETHUSDT` | 5 lines/symbol: bias 4H/1H · 0–2 zones · invalid. Mid-range → **STAND ASIDE** |
...
Read `ticker` + `klines.240` + `klines.60` from `/map` (daily `klines.D` if backfilled). Several names: `GET /map?symbols=BTCUSDT,ETHUSDT,SOLUSDT` → `{ maps: [...] }`. One name stays a single object. Do **not** dump `/brief` 15m into chat. `/brief` unchanged for full 15/60/240.
