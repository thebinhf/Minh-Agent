...
Quant is a veto, not a signal: `tickers[].fundingRate`, `/map.funding.crowded`, `tickers[].openInterest`, `/map.oi.reading` (`long_add`/`short_add`/`cover`/`flush`), `/map.oi.deltaPct`, `/map.liq` (`below` = long-liq under last, `cascade`), `tickers[].price24hPcnt`. Crowded long (`funding.crowded=long`) does not short; it blocks a weak demand arm. `oi.reading=short_add` into a demand zone → stand aside. `liq.cascade` + demand → wait for reclaim, do not catch the knife.
