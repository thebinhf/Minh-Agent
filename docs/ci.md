...
Walk every arm of an A/B on **one pinned window**: export `PAPER_AB_FROM` / `PAPER_AB_TO` once, then run each arm. `--days N` is measured from the clock at each arm's own start, so three 180d arms run 22 minutes apart are three different walks even before the flag does anything — measured on this box: `ticks 658719 / 658751 / 658755` and `quantCoverage.samples 2 517 304 / 2 384 931 / 2 392 056` for baseline / `be` / `scorerr`, part clock, part flag, and impossible to separate after the fact. Pinning the window leaves the flag as the only difference. The `compare` mode of `replay-map-ab.sh` takes the two review JSONs it should subtract.
