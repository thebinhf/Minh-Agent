...
| Proximity ARM | Live | Tick rests post-only OCO when last is in the proximal band of an **accepted** card **and** the last confirmed 15m closes with the zone (still in proximal→entry). Forming/opposite → wait, no reject. Cap 2 busy symbols (`PAPER_ARM_MAX=0` off). Under the cap, rank by family score then `rr` then `zoneId`; occupied slots stay. Tick-snap `rr_below_min` → `rr_fail` latch. Optional TA waits (default off): `PAPER_TA_FIB=arm`, `PAPER_TA_VOL=arm`, `PAPER_TA_SHOCK=arm`, `PAPER_TA_REV=arm`. Missing tape is not a wait. `PAPER_PROXIMITY_ARM=0` / `PAPER_CONFIRM_15=0` off. No chase through entry. |
...
| Paper replay-map | Live | `bun run paper replay-map [SYMBOL] --days 180` (or `--from --to`). Omit symbol = watchlist (10). `--one-book` one equity. `--train-days 90` freezes family floor for the holdout. Walk confirmed 4H detect → MAP policy → 15m ARM/OCO. No future bars. Quant tape **as-of**. `quantCoverage` counts ok vs missing per field (flow/cascade empty window is missing, not 0). `skipReasons` counts MAP denials (`family_floor`, `map_skip`, bias, quant, `rr_fail`, `ta_osc`) separately from ledger `cancelCodes`. Metrics cover the window (not a trailing 7d). Slippage 0. Per-symbol `*-replay-map-SYMBOL.sqlite` unless `--one-book`. Empty tape → `skipped`. Retention `klinesDays` 180. `PAPER_MAP_SKIP` optional (unset = none). |
| Paper review | Live | `bun run paper review FILE.json` — compact QC from a replay-map JSON (one-book / single / watchlist `rows[]`). Flags: `hype_accepted`, `flow_missing`, `cascade_missing`, `tape_skipped`, `quant_missing`, `coverage_absent`. `bun run paper ab BASE.json VARIANT.json` is variant minus base (accepted, skipReasons, equity). Does not walk bars. Lab: [`deploy/replay-map-lab.sh`](../deploy/replay-map-lab.sh). A/B: [`deploy/replay-map-ab.sh`](../deploy/replay-map-ab.sh) (one flag: `baseline` / `chop0` / `floor1` / `arm2` / `arm5` / `arm0` / `skiphype` / `fib` / `osc` / `vol` / `shock` / `rev`). |
