...
| Paper replay-map | Live | `bun run paper replay-map [SYMBOL] --days 180` (or `--from --to`). Omit symbol = watchlist (10). `--one-book` one equity. `--train-days 90` freezes family floor for the holdout. Walk confirmed 4H detect → MAP policy → 15m ARM/OCO. No future bars. Quant tape **as-of**. `skipReasons` counts MAP denials (`family_floor`, `map_skip`, bias, quant, `rr_fail`) separately from ledger `cancelCodes`. Metrics cover the window (not a trailing 7d). Slippage 0. Per-symbol `*-replay-map-SYMBOL.sqlite` unless `--one-book`. Empty tape → `skipped`. Retention `klinesDays` 180. `HYPEUSDT` MAP-skip by default. |
