...
| Liquidation heatmap | Live | Public WS `allLiquidation` (BTC/ETH/SOL) → SQLite `liquidations`. `GET /liq-heatmap`, `/map.liq` (`below`/`above`/`cascade`). Prints only — not estimated leverage. `BYBIT_LIQ=0` off. |
...
| MAP policy agent | Live | `src/agent/` — 4H HH/HL=bull, LH/LL=bear, mixed=chop; 1H must not oppose 4H; 1H chop → stand aside. Mid-range (last between nearest swing H/L and not proximal→entry of a same-direction card) → stand aside; does not close opens. Bull→demand only, bear→supply only; drop RR < account `minRr`, `deep_mitigate`, `htf_break`, expiry; cap 2/symbol. Stale `klineLag` / `tradingAllowed=false` → no accept / no new arm. Paper-only. `/map` and `/zones` contracts unchanged. |
