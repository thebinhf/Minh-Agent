...
| Paper replay-map | Live | `bun run paper replay-map [SYMBOL] --days 180` (or `--from --to`). Omit symbol = watchlist (10). Walk confirmed 4H detect → MAP policy → 15m ARM/OCO. No future bars. Quant tape **as-of**. Metrics cover the window (not a trailing 7d). Slippage 0. Per-symbol `*-replay-map-SYMBOL.sqlite` on watchlist. Empty tape → `skipped`. Retention `klinesDays` 180. |
