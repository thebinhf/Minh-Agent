...
| EVENT desk | Live | `bun run paper event` / `GET /paper/event` — pending OCO + alerts + accepted zones. Bound pending dies with the zone (expire / deep / HTF). Optional `PAPER_BE_R` moves an open SL to entry after favorable MFE ≥ N R (`position.managed` / `be`; default off, A/B). Do not poll `/confirm`. |
...
| Paper review | Live | `bun run paper review FILE.json` — compact QC from a replay-map JSON (one-book / single / watchlist `rows[]`). Flags: `hype_accepted`, `flow_missing`, `cascade_missing`, `tape_skipped`, `quant_missing`, `coverage_absent`. `bun run paper ab BASE.json VARIANT.json` is variant minus base (accepted, skipReasons, equity). Does not walk bars. Lab: [`deploy/replay-map-lab.sh`](../deploy/replay-map-lab.sh). A/B: [`deploy/replay-map-ab.sh`](../deploy/replay-map-ab.sh) (one flag: `baseline` / `chop0` / `floor1` / `arm2` / `arm5` / `arm0` / `skiphype` / `fib` / `osc` / `vol` / `shock` / `rev` / `sd` / `breakout` / `reversal` / `fresh` / `impulse` / `be` / `scorerr`). |
...
| Observe lock | Live | Host `PAPER_OBSERVE=1`: `GET /observe` (feed + gates + last MAP + tape ok/missing + fail-soft live-shadow + paper desk) and `GET /paper/observe`. POST arm/open/zones **403**. CLI mutations reject. MAP/ARM/EVENT still run in-process. Empty CVD/liq window is missing, not 0. |
