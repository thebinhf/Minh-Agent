...
| Hole heal | At boot, before the tail gap-fill: interior cadence holes (tracker-off windows, lost bars) and stale `confirm=0` rows from dead sessions are re-fetched from REST and finalized (`BYBIT_GAP_HEAL`, default on; disable with `=0`). Every repaired candle is venue-served — nothing is invented. |
