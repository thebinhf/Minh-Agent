...
Lab QC (does not walk bars): `bun run paper review FILE.json` compact-reads a `replay-map` JSON (`skipReasons`, `quantCoverage`, flags). Missing flow/cascade is a flag, not a zero. `AGENT_BIAS_CHOP=0` and `proximal` are A/B losers on the 180d tape (~−3130 / ~−2964 equity) — keep deny. 1H chop does not override 4H. `PAPER_ARM_MAX` default 2 (180d: 2 beat 3/5/0). `PAPER_MAP_SKIP=HYPEUSDT` +576 vs keep-1H; default still none. Do not invent historical CVD/liq.
