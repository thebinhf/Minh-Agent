...
| Funnel key | Meaning |
| --- | --- |
| `detected` | Distinct `zoneId`s in the window: ledger accepts **plus** events / orders / positions. `/zones` does not persist detections |
...
Lab QC (does not walk bars): `bun run paper review FILE.json` compact-reads a `replay-map` JSON (`skipReasons`, `quantCoverage`, flags). `bun run paper ab BASE.json VARIANT.json` is variant minus base (one flag at a time; `deploy/replay-map-ab.sh`). Missing flow/cascade is a flag, not a zero. `AGENT_BIAS_CHOP=0` and `proximal` are A/B losers on the 180d tape (~−3130 / ~−2964 equity) — keep deny. 1H chop does not override 4H. `PAPER_ARM_MAX` default 2 (180d: 2 beat 3/5/0). Skip-HYPE +576 vs keep-1H but **−216** vs ARM=2 — default skip stays none. P7 TA gates (`PAPER_TA_FIB` / `AGENT_TA_OSC` / `PAPER_TA_VOL` / `PAPER_TA_SHOCK` / `PAPER_TA_REV`) default **off** until a 180d review wins. P9 arms are decided on a pinned-window, frozen-floor walk: `PAPER_BE_R=0.5` **rejected** (−4 770.6 equity — 68 scratches bought by losing up to 17 of 36 winners) and `PAPER_ZONE_SCORE_RR=1` **no gain** (−84.7). Both stay off. Method that makes a verdict trustworthy: `PAPER_AB_FROM` / `PAPER_AB_TO` for one shared window, `PAPER_LAB_TRAIN_DAYS=90` for a frozen floor, and `paper ab` reporting `methodMismatch: []`. Do not invent historical CVD/liq.
