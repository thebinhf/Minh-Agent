...
| `PAPER_MAP_ALLOCATE` | `feed` | Which card of a symbol takes the last MAP slot when the per-symbol cap (`LEDGER_CAP_PER_SYMBOL` = 2) is the binding gate. `feed` = the order `/zones` listed them (what the desk has always done). `rank` = ARM's own order: family score, then realized RR under `PAPER_ZONE_SCORE_RR`, then card RR, then zoneId. Desk, live-shadow and `replay-map` all run it, so the walk measures the host. A/B before on |
